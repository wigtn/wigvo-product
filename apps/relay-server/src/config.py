...
class Settings(BaseSettings):
    ...
    # Session A(웹/기관 응대) 커밋 에너지 게이트 — 무음/소음만 거른다.
    # 커밋 세그먼트의 peak RMS(pcm16)가 이 값 미만이면 OpenAI 커밋 스킵 + 버퍼 clear.
    # 0으로 두면 비활성.
    #
    # 한때 2000이었다. 근접/원거리를 레벨로 가르려던 값인데, 그 역할은 이제
    # 화자 식별(SpeakerMatcher)이 맡는다. 레벨은 애초에 두 사람을 가르지 못한다 —
    # 본인이 조용히 말하면 원거리와 같은 값이 나오기 때문이고, 실측(2026-07-19
    # 18:24 통화)에서 본인 발화 2건이 peak_rms 1915/1871로 잘려 화자 식별이
    # 판단할 기회조차 없었다. 레벨 게이트가 화자 식별보다 앞에 있어서 생긴 일이다.
    #
    # 그래서 이 게이트는 원래 역할(Whisper 무음 할루시네이션 차단)로 되돌린다.
    # 250은 실측 분포에서 p25=69(무음/소음)는 자르고 실발화는 건드리지 않는 값이다.
